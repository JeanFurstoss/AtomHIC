...
{
	//if( optarg('v')) static const verbose = 
	string InputFilename = argv[1];
	istringstream iss_nmin(argv[2]);
	unsigned int nmin;
	iss_nmin >> nmin;
	istringstream iss_nmax(argv[3]);
	string descriptor_name = "Q";
	unsigned int nmax;
	iss_nmax >> nmax;
	Descriptors MyDescriptors(InputFilename, descriptor_name);
	//KMeans GMM;
	GaussianMixtureModel GMM;
	GMM.setDescriptors(&MyDescriptors);
	GMM.TrainModel(nmin,nmax);
	//ofstream writefile("OutputKMeans.dat");
	//writefile << "NbClust KMeanBIC KMeanLkh GMMBic GMMLkh" << endl;
	//for(unsigned int k=5;k<nbClustMax;k++){
	//	GMM.TrainModel(k);
	//	writefile << k << " " << GMM.getKMeans()->getBIC() << " " << GMM.getKMeans()->getLogLikelihood() << " " << GMM.getBIC() << " " << GMM.getLogLikelihood() << endl;
	//}
	//writefile.close();
	//ofstream writefile2("OutputGMM");
	//for(unsigned int k=0;k<nbClustMax;k++){
	//	writefile2 << GMM.getMu()[k*2] << " " << GMM.getMu()[k*2+1] << " " << GMM.getCov()[k*4] << " " << GMM.getCov()[k*4+1] << " " << GMM.getCov()[k*4+3] << endl;
	//}
	//writefile2.close();
	//GaussianMixtureModel GMM;
	//GMM.setDescriptors(&MyDescriptors);
